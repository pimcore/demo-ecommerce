...
class CheckoutController extends AbstractCartAware
{
    ...
    public function onKernelController(FilterControllerEvent $event)
    {
        ...
        if($checkoutManager->hasActivePayment()) {
            ...
            $url = $this->generateUrl('action', ["controller" => "payment", "action" => "payment", "prefix" => substr($event->getRequest()->getLocale(), 0, 2)]);
    ...
    public function deliveryAction(Request $request)
    {
        ...
        // check errors
        $this->view->errors = $request->get('error') ? explode(',', $request->get('error')) : array();
